...
ext_handlers = {
    ...
    CustomType: lambda obj: umsgpack.Ext(0x30, umsgpack.dumps(list(obj))),
    0x20: lambda ext: complex(*struct.unpack("<ff", ext.data)),
    0x30: lambda ext: CustomType(*umsgpack.loads(ext.data)),
}
...
exported_vars_test_vector = [
    "ext_type_to_class",  # New globals consequent on Python package
    "ext_class_to_type",
    "aload",
    "mp_dump",
    "mp_load",
    "Ext",  # Original namespace
    "PackException",
    "UnpackException",
...
class TestUmsgpack(unittest.TestCase):
    ...
    def test_pack_ext_handler(self):
        for (name, obj, data) in ext_handlers_test_vectors:
            ...
            packed = umsgpack.dumps(obj, ext_handlers=ext_handlers)
    ...
    def test_unpack_ext_handler(self):
        for (name, obj, data) in ext_handlers_test_vectors:
            ...
            unpacked = umsgpack.loads(data, ext_handlers=ext_handlers)
            self.assertEqual(unpacked, obj)
    ...
    def test_ext_handlers_subclass(self):
        ...
        packed = umsgpack.dumps(Square(5), ext_handlers={
            Rectangle: lambda obj: umsgpack.Ext(0x10, umsgpack.dumps([obj.length, obj.width])),
        })
        self.assertEqual(packed, b"\xc7\x03\x10\x92\x05\x05")

        # Test unpack (unpacks base class)
        unpacked = umsgpack.loads(packed, ext_handlers={
            0x10: lambda ext: Rectangle(*umsgpack.loads(ext.data)),
        })
    ...
    def test_namespacing(self):
        ...
        print(exported_vars)
        print('###')
        print(exported_vars_test_vector)
